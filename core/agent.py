...
class Agent:
    ...
    @staticmethod
    def _fix_detail(detail: AnimeDetail, token: str):
    ...
    @staticmethod
    def _fix_video_info(info: VideoInfo, token: str, route_idx: int, ep_idx: int):
    ...
    async def get_video_info(self, token: str, route_idx: int, ep_idx: int) -> Optional[VideoInfo]:
        ...
        self._fix_video_info(info, token, route_idx, ep_idx)
    ...
    async def get_danmaku_metas(self, keyword: str) -> AsyncIterator[DanmakuMeta]:
        for engine in self._danmaku_engines:
            if not config.is_engine_enable(engine.module):
                continue
            async for item in engine.do_search(keyword):
                yield item
